/*
name : 파일명
msg : 기록할 내용
https://3dmpengines.tistory.com/1971
*/
function writeFile(name, msg) {
  var fs = require('fs');
  fs.writeFile(`${name}.txt`, msg, 'utf8', function (err) {
    if (err) throw err;
    console.log('write end');
  });
}

const INIT = 0;
const MEMORY = 6;
const STATE_COUNT = 2 ** MEMORY;

/*
      ************* encoding 과정 트렐리스 다이어그램(인접 행렬)으로 기록 *************
      trellisDiagramBeforeToAfter[before][after] = [input, output]
      trellisDiagramAfterToBefore[after][before] = [input, output]
  */
let trellisDiagramBeforeToAfter = Array(STATE_COUNT)
  .fill()
  .map(() => Array(STATE_COUNT).fill([-1, -1]));

let trellisDiagramBeforeToAfter2 = Array(STATE_COUNT)
  .fill()
  .map(() => Array(2).fill([]));

let visitedBeforeToAfter = Array(STATE_COUNT)
  .fill()
  .map(() => Array(STATE_COUNT).fill(0));

let visitedBeforeToAfter2 = Array(STATE_COUNT)
  .fill()
  .map(() => Array());

// ---

let trellisDiagramAfterToBefore = Array(STATE_COUNT)
  .fill()
  .map(() => Array(STATE_COUNT).fill([-1, -1]));

let trellisDiagramAfterToBefore2 = Array(STATE_COUNT)
  .fill()
  .map(() => Array(2).fill([]));

let visitedAfterToBefore = Array(STATE_COUNT)
  .fill()
  .map(() => Array(STATE_COUNT).fill(0));

let visitedAfterToBefore2 = Array(STATE_COUNT)
  .fill()
  .map(() => Array());

//--

let activeState = Array(MEMORY + 1)
  .fill()
  .map(() => Array(STATE_COUNT).fill(0));

let activeState2 = Array(MEMORY + 1)
  .fill()
  .map(() => Array());

const visitedState = new Set();

function encoderWithBeforeState(mode, input, level, beforeState) {
  let output;
  const temp = [];
  const S = [];
  const afterState = [];

  for (let i = 0; i < MEMORY; i++) {
    S[i] = beforeState[i];
  }

  if (mode == INIT) {
    S[6] = 0x0;
    S[5] = 0x0;
    S[4] = 0x0;
    S[3] = 0x0;
    S[2] = 0x0;
    S[1] = 0x0;
    S[0] = input;
  } else {
    S[6] = S[5];
    S[5] = S[4];
    S[4] = S[3];
    S[3] = S[2];
    S[2] = S[1];
    S[1] = S[0];
    S[0] = input;
  }
  // modulo-2
  temp[0] = (S[0] + S[2] + S[3] + S[5] + S[6]) % 2;
  temp[1] = (S[0] + S[1] + S[4] + S[6]) % 2;
  temp[2] = (S[0] + S[1] + S[2] + S[3] + S[4] + S[6]) % 2;

  for (let i = 0; i < MEMORY; i++) {
    afterState[i] = S[i];
  }

  // binary -> decimal
  output = 4 * temp[0] + 2 * temp[1] + 1 * temp[2];

  const beforeDec = beforeState.reduce(
    (acc, bit, idx) => acc + bit * 2 ** (MEMORY - idx - 1),
    0,
  );
  const afterDec = afterState.reduce(
    (acc, bit, idx) => acc + bit * 2 ** (MEMORY - idx - 1),
    0,
  );
  visitedState.add(beforeDec);
  visitedState.add(afterDec);

  trellisDiagramBeforeToAfter[beforeDec][afterDec] = [input, output];
  visitedBeforeToAfter[beforeDec][afterDec] = 1;
  //   console.log({ input, beforeDec, afterDec, output });

  return afterState;
}

function transposeArray(array, transposedArray) {
  for (let i = 0; i < STATE_COUNT; i++) {
    for (let j = 0; j < STATE_COUNT; j++) {
      transposedArray[j][i] = array[i][j];
    }
  }
}

function main() {
  let beforeState = [0, 0, 0, 0, 0, 0];
  let Q = [];

  for (let i = 0; i < MEMORY + 1; i++) {
    if (i === 0) {
      Q.push([...encoderWithBeforeState(i, 0, i, [0, 0, 0, 0, 0, 0])]);
      Q.push([...encoderWithBeforeState(i, 1, i, [0, 0, 0, 0, 0, 0])]);
      //   console.log('\n' + 2 + '\n');

      const beforeDec = beforeState.reduce(
        (acc, bit, idx) => acc + bit * 2 ** (MEMORY - idx),
        0,
      );
      activeState[i][beforeDec] = 1;
    } else {
      let len = Q.length;
      for (let j = 0; j < len; j++) {
        beforeState = [...Q.shift()];

        Q.push([...encoderWithBeforeState(i, 0, i, beforeState)]);
        Q.push([...encoderWithBeforeState(i, 1, i, beforeState)]);

        const beforeDec = beforeState.reduce(
          (acc, bit, idx) => acc + bit * 2 ** (MEMORY - idx - 1),
          0,
        );
        activeState[i][beforeDec] = 1;
      }
      //   console.log('\n' + len * 2 + '\n');
    }
  }

  transposeArray(trellisDiagramBeforeToAfter, trellisDiagramAfterToBefore);
  transposeArray(visitedBeforeToAfter, visitedAfterToBefore);

  // 두번째 자료형에 저장

  //   for (let before = 0; before < trellisDiagramBeforeToAfter.length; before++) {
  //     for (let after = 0; after < trellisDiagramBeforeToAfter.length; after++) {
  //       const [input, output] = trellisDiagramBeforeToAfter[before][after];

  //       if (input !== -1) {
  //         trellisDiagramBeforeToAfter2[before][input] = [after, output];
  //         visitedBeforeToAfter2[before].push(after);
  //       }
  //     }
  //   }

  //   for (let after = 0; after < trellisDiagramAfterToBefore.length; after++) {
  //     for (
  //       let before = 0;
  //       before < trellisDiagramAfterToBefore.length;
  //       before++
  //     ) {
  //       const [input, output] = trellisDiagramAfterToBefore[after][before];

  //       if (input !== -1) {
  //         trellisDiagramAfterToBefore2[after][input] = [before, output];
  //         visitedAfterToBefore2[after].push(before);
  //       }
  //     }
  //   }

  //   for (let level = 0; level < MEMORY + 1; level++) {
  //     for (let state = 0; state < STATE_COUNT; state++) {
  //       if (activeState[level][state]) activeState2[level].push(state);
  //     }
  //   }

  //   const minusOneArray = Array(STATE_COUNT).fill(-1);

  //   activeState2 = activeState2.map(array =>
  //     [...array, ...minusOneArray].slice(0, STATE_COUNT),
  //   );

  //   console.log(activeState2);

  trellisDiagramBeforeToAfter2.forEach(before =>
    before.forEach(array => {
      if (!array.length) array.push(-1, -1);
    }),
  );

  visitedBeforeToAfter2.forEach(before => {
    if (!before.length) before.push(-1, -1);
  });

  trellisDiagramAfterToBefore2.forEach(after =>
    after.forEach(array => {
      if (!array.length) array.push(-1, -1);
    }),
  );

  visitedAfterToBefore2.forEach(after => {
    if (!after.length) after.push(-1, -1);
  });

  //   console.log(trellisDiagramBeforeToAfter2);
  //   console.log(visitedBeforeToAfter2);

  //   console.log(trellisDiagramAfterToBefore2);
  //   console.log(visitedAfterToBefore2);

  // console.log(
  //   activeState.map(array => array.reduce((acc, cur) => acc + cur, 0)),
  // );

  console.log(
    activeState.map(array =>
      array.reduce((acc, cur, idx) => (cur ? [...acc, idx] : acc), []),
    ),
  );

  //   console.log(
  //     visitedBeforeToAfter.map(col => col.reduce((acc, row) => acc + row, 0)),
  //   );

  writeFile(
    'visitedBeforeToAfter',
    JSON.stringify(visitedBeforeToAfter)
      .replaceAll('[', '{')
      .replaceAll(']', '}\n'),
  );
  writeFile(
    'trellisDiagramBeforeToAfter',
    JSON.stringify(trellisDiagramBeforeToAfter)
      .replaceAll('[', '{')
      .replaceAll(']', '}\n'),
  );
  writeFile(
    'trellisDiagramAfterToBefore',
    JSON.stringify(trellisDiagramAfterToBefore)
      .replaceAll('[', '{')
      .replaceAll(']', '}\n'),
  );
  writeFile(
    'visitedAfterToBefore',
    JSON.stringify(visitedAfterToBefore)
      .replaceAll('[', '{')
      .replaceAll(']', '}\n'),
  );

  writeFile(
    'activeState',
    JSON.stringify(activeState).replaceAll('[', '{').replaceAll(']', '}'),
  );
  //   console.log(activeState);

  return trellisDiagramBeforeToAfter;
}

main();
// console.log(JSON.stringify(main()).replaceAll('[', '{').replaceAll(']', '}'));
